	...
	// enroll ok
		...
		// insert success
			...
			if(!$result = mysqli_query($conn,$sql)){
				$to_client->setDebugs("select review table error", $sql);
			}
			if($row = mysqli_fetch_assoc($result)){
			// match success
				$to_client->setFields('store_id', $row['pid']);
				$to_client->setFields('state', CLIENT_REVIEW_DATA_ENROLL_SUCCESS);
			...
			// gcm to this user's followers
				...
				curl_close($ch);
				$to_client->setDebugs('gcm_curl', $response);
				
			}
			else{
			// match fail
				$to_client->setDebugs('do_gcm', 'fail');
		...
		$to_client->setFields('state', CLIENT_REVIEW_DATA_ENROLL_EXIST);
